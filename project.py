...
#' Arteries can be approximated by tubes of various sizes.  Pulmonary arteries can vary from ~.3cm to 1.5cm in diameter [1].  Arteries are surrounded by elastin, a protein which gives the artery elastic properties and stiffens with age [2].  While arteries are elastic, the model for arterial flow is much simpler assuming the artery is a rigid tube.  Additionally, arteries can't necessarily be represented as tubes due to the many bifurcations and changes in shape [3]. Thus fluid flow in arteries is mostly determined with a comnination of flow data from ultrasound or MRI, and computational analysis.

#' Additionally, blood is a non-newtonian fluid that is best represented by the Casson model: $ \sqrt{\tau} = \sqrt{\mu} \sqrt{\gamma} + \sqrt{\tau_0}$, however for this analysis, I will assume the fluid is newtonian with a density of viscosity of 4 cP, and constant density of 1.06 $g/cm^3$
...
#' ###### Oscillatory
#' For oscillatory flow, we can assume that velocity is only in the z-direction, and a function or radius and time only.  Additionally, we can assume that radial and angular velocity are both 0, and that gravity in the z-direction is 0.  Under these coditions, the z-component of the Navier-Stokes equations simplify to the following: $$\rho \frac{\partial v_z}{\partial t}  = -     \frac{\partial p}{\partial z} + \mu \left[    \frac{1}{r}\frac{\partial}{\partial r}\left(r     \frac{\partial v_z}{\partial r}\right)\right]$$

#' By assuming a pressure form of $\frac{d p}{d z} = -p_0 e^{i n \omega t}$ and assuming that velocity follows a similar form, we can perform a separation of variables as well as dimensional analysis to end up with the Bessel differential equation: $$0 = \zeta^2 \frac{d^2 V(\zeta)}{d \zeta^2} + \zeta\frac{d V(\zeta)}{d \zeta} +  (\zeta^2 - \alpha^2) V(\zeta)  \textrm{, Where $\alpha = 0$}$$
...
#'
#' ###### Pulsatile
#' Pulsatile is then the linear combination of the steady and oscillatory profiles, giving the following velocity: $$ v_z(\psi,t) = \frac{a^2}{4 \mu}     \left( C\right) \left( 1 - \psi^2\right) + \Re \left(\sum_{n=1}^N \frac{i D C_n a^2}{ \mu \Omega_n^2 } \left( 1 - \frac{J_0(\Lambda_n \psi)}{J_0(\Lambda_n)}\right) e^{i (n \omega t - \Phi_n)}\right)$$ where $\psi = \frac{r}{a}$ when driven with a pressure wave:
#'$$\frac{d p}{d z} = -C - D \sum_{n=1}^N C_n \cos(\frac{2 n \pi t}{T}+ \Phi_n)$$

#' The volumetric flow rate can then be calculated similarly to above: $$ Q = \frac{\pi C a^4}{8 \mu} + \Re \left(\sum_{n=1}^N \frac{\pi D C_n a^4}{\mu \Omega_n^2}\left(1 - \frac{2 J_1(\Lambda_n)}{\Lambda_n J_0(\Lambda_n)}\right)e^{i(n \omega t - \Phi_n)} \right)$$

#' And similarly, the wall shear stress: $$ \tau_{rz} =C\frac{a}{2} +  \Re \left(\sum_{n=1}^N \frac{D C_n a}{\Lambda_n} \frac{J_1(\Lambda_n)}{J_0(\Lambda_n)}e^{i(n \omega t - \Phi_n)}\right)$$

#' ##### Simulation
#' The simulation was done using python and the packages scipy, numpy, and matplotlib.  The main goal of the simulation was to visualize the flow in real time, as well as determie the relative effects of internal and viscous effects in pulsatile flow which show up in the Reynolds number and Womersley number.
...
#' **Figure 1:** Steady flow velocity, shear, and volumetric flow rate for a pressure gradient of 46 $dyne/cm^3$ and radius 0.65 $cm$ (left). Pressure gradient of 100 $dyne/cm^2$ and a radius of 0.3  $cm$ (right).  **Table 1:** Table of Reynolds numbers for the two conditions shown in Figure 1.  The first condition has a much larger Reynolds number since the radius of the pipe is larger allowing a faster maximum velocity.
...
#' In these two conditions of oscillatory flow, the profiles look different mostly due the difference in the radius of the tubes affecting the Womersley number of each flow.  The first condition again has the highest amplitude flow rate due to the larger size of the tube but the difference between the two sizes is much smaller. Similarly the wall shear stresses are similar orders of magnitude, but the smaller tube has larger wall shear stress.
...
#+ results='raw',fig = False,echo = False
T = .8
a = 0.65
